...
- `bash scripts/build-codebuild-source.sh` — re-bundles `per-customer-source.zip` for `tower-per-customer` codebuild
...
| `stopped` ⇄ `active` | the operator's `deploy.sh stop` (the export, the row, the destroy) and `start` (the apply again). Nothing scheduled, nothing mailed; the settings go with the stack and the onboarding line restores them | `TF_ACTION=stop` / `apply` | `prod/tower/AGENTS.md` § one account, many stacks |
