...
- `terraform validate` — run from `modules/*/infra/`; every directory at once, in parallel, is `bash .github/workflows/tf-validate-all.sh`, which `.github/workflows/terraform.yaml` runs on every pull request after `terraform fmt -check -recursive` (format with `terraform fmt -recursive`). A lock file carries `h1:` for `darwin_amd64` and `linux_amd64` (`terraform providers lock -platform=darwin_amd64 -platform=linux_amd64`), so the runner's plugin cache verifies; `tests/tower/local/test_codebuild_source.py` checks each has both
