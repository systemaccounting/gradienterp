...
- `bash scripts/dispatch.sh deploy.yaml|apply.yaml [-f key=value …]` — start a GitHub workflow run and wait on it, one exit status for the run and the failing steps' log on a failure. `deploy.yaml` (`gerp`, `source_version`) pushes the dirs an uploaded `source.zip` names and runs their modules' integ tests; `apply.yaml` (`stack`, `gerp`, `action`, `region`, `source_version`, `plan_only`) runs that tree's `apply.sh`. Both take the zip `zip.sh source` + `upload.sh source` put, committed or not, and run in the `prod` environment through `gerp-github-deploy` (`.github/actions/aws`)
