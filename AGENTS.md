...
- **money is sliced by two dimensions, location and job.** a location is an ordinal from settings' `LOCATION#` registry (`modules/settings/AGENTS.md` § locations states the contract every emitting module keeps: a placeful id leads with the ordinal, a posting path copies its own row's `location` into `dimensions.location`, `post_journal_entry` stamps `"1"` when absent, a tool declares `location` wherever its handler reads it). a job is a tag the owner picks, no object behind it. `get_statement` slices by either
