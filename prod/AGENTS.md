...
- **`per_customer/`** — a *template*, applied once per customer (the canonical bring-up; `scripts/customer_apply.sh` is obsolete). Takes `customer_id` + `customer_account_id`; looks up tenant metadata from SSM (`/gradienterp/customers/<customer_id>`); wires `server` + `agent` + `schemas` + `accounting` + `contacts` + `notes` + `tasks` + `calendar` + `inventory` (payments / labor / purchasing / invoicing / treasury / iot as they land). Order matters: server provisions the shared API; agent provisions the AgentCore runtime + Gateway and writes gateway SSM params; schemas owns the per-customer registry DDB; domain modules read `module.schemas.schema_table_name` for cold-start validation and register their tools on the gateway. One tfstate per customer at `s3://<operator-state-bucket>/<customer_id>/terraform.tfstate`. Customer B's apply never refreshes A's state.
