...
| `aws_ecr_repository.agent` (`agentcore`) | operator-shared agent container image. immutable tags, Amazon Inspector scanning (on push and continuously, HIGH/CRITICAL findings to `gerp-ops-alerts`), keep-last-10 lifecycle, org-scoped pull policy. every customer's AgentCore Runtime pulls cross-account; per-customer terraform reads the most recent image and never builds. (`agent_image.tf`) |
