...
#
# **Off by default.** `closure_requester_accounts` is empty, so the role trusts nobody until an
# account is named. Same posture as `close_build_project` — the machinery exists and does nothing
...
variable "closure_requester_accounts" {
  description = "AWS account ids allowed to assume the closure-requester role. Empty (the default) trusts nobody. gradienterp's own account goes here when its collection sequence goes live; a customer's gerp never appears."
  type        = list(string)
  default     = []
}

resource "aws_iam_role" "closure_requester" {
  count = length(var.closure_requester_accounts) == 0 ? 0 : 1
  ...
  assume_role_policy = jsonencode({
    ...
    Statement = [{
      ...
      Principal = { AWS = [for a in var.closure_requester_accounts : "arn:aws:iam::${a}:root"] }
      Condition = {
...
output "closure_requester_role_arn" {
  ...
  value       = length(var.closure_requester_accounts) == 0 ? "" : aws_iam_role.closure_requester[0].arn
}
