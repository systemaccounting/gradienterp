...
| `aws_controltower_landing_zone` (v4.0) | CT landing zone governing every `REGIONS` region (a region added there is a landing-zone update, tens of minutes, and Config recording in it for every enrolled account). The landing zone returns its regions in its own order whatever order they're sent in, so `regions.tf` sends that order (`landing_zone_region_order`); a new region goes last and joins the list once AWS shows where it put it. Manages: baseline CloudTrail in log_archive, Config recorders + aggregator in audit, IC integration, baseline preventive controls. |
