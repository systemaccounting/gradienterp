...
- **`agent`** — AgentCore runtime + gateway + memory; pulls the most recent operator-shared image (`data.aws_ecr_image`, `most_recent`; `deploy.sh image` moves a live runtime). Also serves the **web chat front door** (`lambdas/chat` + a Function URL) when cognito is wired — takes `cognito_user_pool_id`/`cognito_client_id` + a constructed `contacts_table_name` (constructed, not `module.contacts`'s output: agent applies before contacts). See `modules/agent/AGENTS.md` §web chat.
...
- **an existing gerp**: `bash scripts/apply.sh --stack per_customer --gerp <id>|all [--action stop] [--plan]` — the same `tower-per-customer` build, from the latest `upload.sh source` (`--build` zips and uploads this tree first); `apply.yaml` runs it from GitHub
