...
- **the second rule on the firm's bus** (`infra` rule `to_operator`, role `to_operator`) — the same `source = metrics` event rule 1 stores is put on the operator's bus (`gerp-operator`, us-east-1) as recorded, no transformer (a cross-account bus target takes none) and no function. Not through the hub: a bus target is taken once per event (`THIRD_ACCOUNT_HOP_DETECTED`), and the hub's forward edge would be the second. The operator's `metrics` rule counts it under the firm's partition when the gerp's row reads `published`, and drops it otherwise. Nothing is published: an openly operated firm's every product event counts, at day, week and month, `count` and `active`, from the event's name, `ts` and `zone`. `subject_id` crosses as recorded and is never served: the api answers a set's size, and a channel drops it by its contract class `subject`
