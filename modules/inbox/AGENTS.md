...
- **the sender is verified at the door** — `detail.from` is the sender's word, and any account in the organization can put on a hub bus. `receive_inbound` resolves `detail.from` in the platform directory (`DIRECTORY_TABLE_ARN`, the operator's `gerp-directory` in this region) and the row's `aws_account_id` has to be the event's `account` (EventBridge stamps it; the hub's delivery keeps it). A verified event lands `received` with `from_gerp`; any other lands `refused` with `claimed_from` and `refused_reason` (`sent from another account`, `the directory doesn't know the sender`, `no sending account`, `no sender named`), logged as `inbound refused`. The router hands a refused row to nobody, and the handlers (`agreements`, `treasury`, `shipping`) read the sender only from `from_gerp`. In Lambda with no directory wired every event is refused; off Lambda (the local stack) the claim is taken.
