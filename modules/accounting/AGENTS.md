...
- `get_statement {statement: trial_balance|income|balance_sheet}` — statement reads over a time range; all three take an optional `dimensions` filter (subset match on each row's entry-level dims — `{"location": "2"}` = one location's slice; an entry stamped with a different value or with none of a filter's other keys belongs to no slice, so a filtered balance sheet is a view and need not balance). Unfiltered = consolidated, straight off the ledger fold
- `post_journal_entry` stamps `dimensions.location = "1"` when the caller's map has no `location` key (main, the default by doctrine), so every entry is in some location's slice. A posting path copies its own row's `location`; nothing reads the settings table at post time
...
- each journal line is recorded as a metrics event, `<type>.posted` for the five account types, the entry the subject, `amount` signed by the type's normal balance, `via: ledger` (modules/metrics `record`: the firm's own bus, and the platform when openly operated); `journal_entry.posted` goes out through `events.publish`, so a private firm's posting is withheld at the source; the economic counters `post_journal_entry` stamps on it (`detail.counters`, `{op, key, magnitude}` each — translate at the boundary; the operator's counter lambda only adds): `revenue` from credit legs to revenue accounts, `expense` from debit legs to expense accounts, cost of goods sold included — the two the economy's margin is computed from
