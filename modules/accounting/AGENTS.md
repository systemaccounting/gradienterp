...
- the economic counters `post_journal_entry` stamps on `journal_entry.posted` (`detail.counters`, `{op, key, magnitude}` each — translate at the boundary; the operator's counter lambda only adds): `revenue` from credit legs to revenue accounts, `expense` from debit legs to expense accounts, cost of goods sold included — the two the economy's margin is computed from
