...
- **`events.py`** — the one place an event is built and sent, bundled into an emitting lambda by the same import-glob tier that carries `aws.py`. Four functions. Three by REACH, so a caller picks the bus by which one it calls rather than by which env var it reaches for: `emit(source, detail_type, detail)` on the firm's own bus, `emit_to(source, recipient, …)` addressed on the shared bus (stamps `from`/`to`), `publish(source, …)` on the shared bus (stamps `schema_version` / `openly_operated` / `customer_id`). It owns local mode, and it NEVER raises: emission is downstream of the durable write, so a failure to announce must not report the write as failed. A `put_events` failure prints the line `create_inc_from_log` reads, keyed `event:<source>.<detail-type>` — a bus refusing every emit of a type collapses into one incident instead of thousands of log lines. `publish` sends on condition, `if oob: send`: the flag is read per invoke and a private firm's call puts nothing and returns `withheld`, so every platform copy of a firm's event goes through it (accounting's postings, metrics records). `put_shared(source, detail_type, detail)` is the envelope without the condition, for the one control message that leaves when the flag is off: the flip itself, `gerp.unpublished` (modules/settings).
...
| `events.publish` from `metrics.record` (source `metrics`) | `metrics` / the event's own name | the hub, the event as recorded (`customer_id`, `zone`) plus the envelope, a published firm's only; the hub forwards it to the operator, whose counter counts it under the firm's partition |
...
- **`openly_operated`** — the archive rule's filter (`{"detail":{"openly_operated":[true]}}`), stamped by the envelope. Always true on a published event, since `publish` withholds a private firm's; the one event that carries false is the flip, `gerp.unpublished`, put by `put_shared`. Nothing reads the archive; the stream gates on the gerp row's `published` instead, so the stamp is on its way out (`prod/api_openlyoperated/TODO.md`).
