"""Shared helpers for tests/assets/local/."""
...
LAMBDAS_DIR = REPO_ROOT / "modules" / "assets" / "lambdas"
...
def load_lambda(name):
    ...
    for mod_name in list(sys.modules):
        if mod_name in ("_helpers",) or mod_name.startswith("lambda_assets_"):
            del sys.modules[mod_name]
    path = LAMBDAS_DIR / name / "main.py"
    spec = importlib.util.spec_from_file_location(f"lambda_assets_{name}", path)
...
def scratch_env():
    ...
    # journal entry validates against, since assets's entry dispatches in-process to the real
    # post_journal_entry (modules/aws/aws.py). ONE schema table carries both registries: the
    # chart books() seeds and assets's own field registry, exactly as a deployment has it.
    bk = books(name)
    seed_registry(bk["SCHEMA_TABLE"], "asset_fields")

    overrides = {
        **bk,
        "ASSETS_TABLE": make_table("assets"),
