...
only when its row reads `published`, the detail projected through the event's contract. Nothing goes
out for an event sent from any account but the one on the gerp's row."""
...
def _acct(gerp_id):
    """The account a gerp's events are sent from, as EventBridge stamps it."""
    return "1" + str(abs(hash(gerp_id)) % 10**11).zfill(11)


def _row(gerp_id, published=None):
    from aws import client
    item = {"gerp_id": {"S": gerp_id}, "status": {"S": "active"}, "aws_account_id": {"S": _acct(gerp_id)}}
...
def _posted(gerp_id, counters=True):
    ...
    return {"detail-type": "journal_entry.posted", "time": "2026-09-01T00:00:00Z", "account": _acct(gerp_id), "detail": detail}
...
def test_counters_go_out_for_every_gerp_and_the_channel_only_when_published():
    with scratch_env():
        ...
        # a gerp with no row at all is no gerp the operator counts
        sent.clear()
        assert mod.handler(_posted("ghost"), None) == {"published": []}
...
def test_a_contract_drops_subjects_and_secrets_and_a_kind_without_one_has_no_channel():
    with scratch_env():
        mod, sent = _load()
        _row("cafe", True)
        paid = {"detail-type": "distribution.paid", "time": "2026-09-01T00:00:00Z", "account": _acct("cafe"), "detail": {
        ...
        sent.clear()
        unknown = {"detail-type": "something.happened", "account": _acct("cafe"), "detail": {"customer_id": "cafe", "x": 1}}
        assert mod.handler(unknown, None) == {"published": []}
        assert sent == []
        # a bare counters event from a kind with no contract still reaches the aggregate
        assert mod.handler({"detail-type": "something.happened", "account": _acct("cafe"), "detail": {"customer_id": "cafe", "counters": [{"key": "expense", "magnitude": 3}]}}, None) == {"published": [{"channel": "/oob/counters", "events": 1}]}
...
def test_an_event_from_another_account_puts_nothing_out():
    """Any account in the organization can put on the bus. An event naming a gerp from any account but
    the one on its row — or from none — reaches neither the gerp's channel nor the aggregate."""
    with scratch_env():
        mod, sent = _load()
        _row("cafe", True)
        _row("mallory", True)
        for account in (_acct("mallory"), ""):
            spoofed = _posted("cafe")
            spoofed["account"] = account
            assert mod.handler(spoofed, None) == {"published": []}, account
        assert sent == []
        assert mod.handler(_posted("cafe"), None)["published"], "the gerp's own account still goes out"


def test_a_value_the_contract_gives_no_shape_is_dropped_not_passed_whole():
    """A property typed as a scalar that arrives as an object carries whatever the sender put in it."""
    with scratch_env():
        mod, _ = _load()
        schema = {"properties": {"amount": {"type": "number"}, "memo": {"type": "string"},
                                 "lines": {"type": "array", "items": {"properties": {"n": {"type": "number"}}}}}}
        detail = {"amount": {"ssn": "123-45-6789"}, "memo": "ok", "lines": [{"n": 1, "who": "Ana"}, "stray"]}
        assert mod.project(detail, schema) == {"memo": "ok", "lines": [{"n": 1}, "stray"]}
        assert mod.project({"memo": ["a", "b"]}, schema) == {}


def test_an_operator_contract_has_no_public_channel():
    with scratch_env():
        mod, sent = _load()
        _row("cafe", True)
        ext = {"detail-type": "registry.extended", "account": _acct("cafe"), "detail": {
            "schema_version": 1, "openly_operated": True, "customer_id": "cafe", "registry": "contact_fields",
            "bucket": "customer", "name": "fitment", "reason": "parts shop"}}
        assert mod.contract("registry.extended")["audience"] == "operator"
        assert mod.handler(ext, None) == {"published": []}
        assert sent == []
