...
def test_playbooks_runs_on_a_kb_change_by_dispatch_or_by_call_and_finds_the_knowledge_base_by_name():
    ...
    # selective: what each gerp last synced is a settings row, `{path: sha256}`; a playbook whose hash
    # differs is the ingest set, so a squash merge or an uncommitted edit changes nothing about it
    for needle in ('"GERP#playbooks"', "sha256", "export ONLY", "put-item"):
        assert needle in script, needle
    assert "git " not in script, "no history: the stamp is content"
