...
#
#   bash scripts/apply.sh --stack per_customer --gerp <gerp_id>|all [--action apply|stop] [--plan]
#   bash scripts/apply.sh --stack hub --region <region> [--plan]
...
# another); the operator stacks run terraform in place, as the default profile. See scripts/apply.py.
set -euo pipefail
cd "$(dirname "$0")/.."
PY=python3
